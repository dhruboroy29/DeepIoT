...
prob_list_acc1 = tf.get_variable("prob_list_acc1", [1, 1, 1, CONV_NUM], tf.float32,
                                 tf.constant_initializer(CONV_KEEP_PROB), trainable=False)
prob_list_acc2 = tf.get_variable("prob_list_acc2", [1, 1, 1, CONV_NUM], tf.float32,
                                 tf.constant_initializer(CONV_KEEP_PROB), trainable=False)
prob_list_gyro1 = tf.get_variable("prob_list_gyro1", [1, 1, 1, CONV_NUM], tf.float32,
                                  tf.constant_initializer(CONV_KEEP_PROB), trainable=False)
prob_list_gyro2 = tf.get_variable("prob_list_gyro2", [1, 1, 1, CONV_NUM], tf.float32,
                                  tf.constant_initializer(CONV_KEEP_PROB), trainable=False)
prob_list_senIn = tf.get_variable("prob_list_senIn", [1, 1, 1, 1, CONV_NUM], tf.float32,
                                  tf.constant_initializer(CONV_KEEP_PROB), trainable=False)
prob_list_sen1 = tf.get_variable("prob_list_sen1", [1, 1, 1, 1, CONV_NUM2], tf.float32,
                                 tf.constant_initializer(CONV_KEEP_PROB), trainable=False)
prob_list_sen2 = tf.get_variable("prob_list_sen2", [1, 1, 1, 1, CONV_NUM2], tf.float32,
                                 tf.constant_initializer(CONV_KEEP_PROB), trainable=False)
prob_list_sen3 = tf.get_variable("prob_list_sen3", [1, 1, 1, 1, CONV_NUM2], tf.float32,
                                 tf.constant_initializer(CONV_KEEP_PROB), trainable=False)
prob_list_rnn1 = tf.get_variable("prob_list_rnn1", [1, INTER_DIM], tf.float32, tf.constant_initializer(RNN_KEEP_PROB),
                                 trainable=False)
prob_list_rnn2 = tf.get_variable("prob_list_rnn2", [1, INTER_DIM], tf.float32, tf.constant_initializer(RNN_KEEP_PROB),
                                 trainable=False)
...
prob_list_dict = {u'acc_conv1': prob_list_acc1, u'acc_conv2': prob_list_acc2, u'gyro_conv1': prob_list_gyro1,
                  u'gyro_conv2': prob_list_gyro2,
                  u'acc_conv3': prob_list_senIn, u'gyro_conv3': prob_list_senIn, u'sensor_conv1': prob_list_sen1,
                  u'sensor_conv2': prob_list_sen2,
...
def deepSense(inputs, train, reuse=False, name='deepSense'):
    with tf.variable_scope(name, reuse=reuse) as scope:
        ...
        acc_conv1, acc_dropB1 = DeepIoT_dropOut.dropout(acc_conv1, dropOut_prun(prob_list_acc1, prun_thres, sol_train),
                                                        is_training=train,
                                                        noise_shape=[acc_conv1_shape[0], 1, 1, acc_conv1_shape[3]],
                                                        name='acc_dropout1')
        ...
        acc_conv2, acc_dropB2 = DeepIoT_dropOut.dropout(acc_conv2, dropOut_prun(prob_list_acc2, prun_thres, sol_train),
                                                        is_training=train,
                                                        noise_shape=[acc_conv2_shape[0], 1, 1, acc_conv2_shape[3]],
                                                        name='acc_dropout2')
        ...
        gyro_conv1, gyro_dropB1 = DeepIoT_dropOut.dropout(gyro_conv1,
                                                          dropOut_prun(prob_list_gyro1, prun_thres, sol_train),
                                                          is_training=train,
                                                          noise_shape=[gyro_conv1_shape[0], 1, 1, gyro_conv1_shape[3]],
                                                          name='gyro_dropout1')
        ...
        gyro_conv2, gyro_dropB2 = DeepIoT_dropOut.dropout(gyro_conv2,
                                                          dropOut_prun(prob_list_gyro2, prun_thres, sol_train),
                                                          is_training=train,
                                                          noise_shape=[gyro_conv2_shape[0], 1, 1, gyro_conv2_shape[3]],
                                                          name='gyro_dropout2')
        ...
        sensor_conv_in, sensor_in_dropB = DeepIoT_dropOut.dropout(sensor_conv_in,
                                                                  dropOut_prun(prob_list_senIn, prun_thres, sol_train),
                                                                  is_training=train,
                                                                  noise_shape=[senor_conv_shape[0], 1, 1, 1,
                                                                               senor_conv_shape[4]],
                                                                  name='sensor_dropout_in')
        ...
        sensor_conv1, sensor_dropB1 = DeepIoT_dropOut.dropout(sensor_conv1,
                                                              dropOut_prun(prob_list_sen1, prun_thres, sol_train),
                                                              is_training=train,
                                                              noise_shape=[sensor_conv1_shape[0], 1, 1, 1,
                                                                           sensor_conv1_shape[4]],
                                                              name='sensor_dropout1')
        ...
        sensor_conv2, sensor_dropB2 = DeepIoT_dropOut.dropout(sensor_conv2,
                                                              dropOut_prun(prob_list_sen2, prun_thres, sol_train),
                                                              is_training=train,
                                                              noise_shape=[sensor_conv2_shape[0], 1, 1, 1,
                                                                           sensor_conv2_shape[4]],
                                                              name='sensor_dropout2')
        ...
        sensor_conv3, sensor_dropB3 = DeepIoT_dropOut.dropout(sensor_conv3,
                                                              dropOut_prun(prob_list_sen3, prun_thres, sol_train),
                                                              is_training=train,
                                                              noise_shape=[sensor_conv3_shape[0], 1, 1, 1,
                                                                           sensor_conv3_shape[4]],
                                                              name='sensor_dropout3')
        ...
        gru_bMask1 = DeepIoT_rnnDrop.GenRNNMask(keep_prob=dropOut_prun(prob_list_rnn1, prun_thres, sol_train),
                                                is_training=train, batch_size=BATCH_SIZE, inter_dim=INTER_DIM)
        ...
        gru_bMask2 = DeepIoT_rnnDrop.GenRNNMask(keep_prob=dropOut_prun(prob_list_rnn2, prun_thres, sol_train),
                                                is_training=train, batch_size=BATCH_SIZE, inter_dim=INTER_DIM)
...
batch_eval_feature, batch_eval_label = input_pipeline_har(os.path.join(TF_RECORD_PATH, 'eval.tfrecord'), BATCH_SIZE,
...
movingAvg_batchLoss = tf.get_variable("movingAvg_batchLoss", [], tf.float32, tf.constant_initializer(0.0),
                                      trainable=False)
movingStd_batchLoss = tf.get_variable("movingStd_batchLoss", [], tf.float32, tf.constant_initializer(1.0),
                                      trainable=False)
